...
app.add_middleware(
    CORSMiddleware,
    allow_origins=origins,           # Permite las peticiones desde tu frontend
...
@app.post("/api/v1/implant-cards", response_model=ImplantCardResponse, status_code=status.HTTP_201_CREATED)
async def create_implant_card(payload: ImplantCardCreate, db: Session = Depends(get_session)):
    """
    ...
    db_record = ImplantCard.model_validate(payload, update={"risk_class": predicted_tier})
...
@app.get("/api/v1/implant-cards", response_model=List[ImplantCardResponse], status_code=status.HTTP_200_OK)
